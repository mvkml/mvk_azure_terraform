...
variable "storage_accounts" {
  ...
  default = [ {
    Id                       = 1
    # Name of the storage account, must be globally unique across Azure
    # saetnaubcliamscus27
    # sa storage account 
    # etna client
    # ubcliam storage account name 
    # cus location
    # dev environment
    # 27 version or build number
    # sa-etna-ubclaims-cus-27
    # saetnaubclaimsdev01
    name                     = "saetnahrcus01"
        ...
        blobs = [
          {
            ...
            container_id   = 101
            sa_account_id  = 1
            container_name = "hr-process"
            name           = "preprocess"
            type           = "Block"
            source         = "documents/ai_blobs" #default_file
            pattern        = "*.*"
            blob_items     = []
            description    = "For HR development and testing purposes"
            is_active      = true
          },
          {
            Id             = 1002
            container_id   = 102
            sa_account_id  = 2
            container_name = "hcfa-claims"
            name           = "preprocess"
            type           = "Block"
            source         = "documents/ai_blobs" #default_file
            pattern        = "*.*"
            blob_items     = []
            description    = "For HCFA development and testing purposes"
            is_active      = false
          }
        ]
      }
...
variable app_context {
  type        = object({
    ...
    secondary_location       = optional(string, "Central India")
